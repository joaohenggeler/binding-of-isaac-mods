...
local mod = RegisterMod("strawpack 2 - seed of discord", 1)

local HelperFunctions = require("resources.scripts.strawpack helper functions")
local DataLoaderAndSaver = require("resources.scripts.strawpack data loader and saver")
...
local ModData = DataLoaderAndSaver.get_seed_of_discord_data()
...
local function on_game_start(_, from_save)
	...
	if not from_save then -- run start (Seeds from the eggs menu can only be manually picked before a new run starts)
		...
		ModData.OriginalSeedEffects = HelperFunctions.get_seed_effects()
		DataLoaderAndSaver.save_mod_data() -- save them because the trinket can mess with seeds and the run's challenge
	end
end
...
return {
	TRINKET_ID = SeedOfDiscord.TRINKET_SEED_OF_DISCORD
}